...
For `nlme`, only models with `varIdent` have been tested.  
...
May include `glmmTMB` and/or `brms` at some point, but for these, you can
extract any parameters associated with sigma model for distributional models
using the `component` argument of the other standard functions.


### Extract correlation structure

With some packages one can estimate the residual correlation structure.  Right now this works for nlme and brms objects.
